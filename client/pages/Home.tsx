...
const steps = [
  {
    ...
    description: "List every subject and how many weekly touchpoints each class requires.",
  },
  {
    label: "Step 2",
    title: "Set sections & days",
    description: "Choose the sections you teach and the instructional windows available.",
  },
  {
    label: "Step 3",
    title: "Generate & refine",
    description: "Produce a conflict-free teacher plan, adjust details, then share with your team.",
  },
];
      ...
      <section className="relative overflow-hidden rounded-[2.75rem] border border-white/40 bg-white/80 p-10 shadow-card sm:p-14 lg:p-16">
        ...
        <div className="relative flex flex-col gap-10 lg:flex-row">
          <div className="flex-1 space-y-6">
            <span className="chip w-max animate-fade-up">Teacher-first scheduling</span>
            ...
            <p className="max-w-xl text-base text-foreground/75 md:text-lg">
              Build a teaching timetable that balances every section, honours instructional minutes, and ensures your
              faculty is never double-booked. Configure once, then iterate with live editing and exports.
            </p>
          ...
          <div className="flex flex-1 flex-col gap-4 rounded-[2rem] bg-white/55 p-6 shadow-glow backdrop-blur">
            <h2 className="font-display text-xl font-semibold text-foreground">Preview your faculty rotation</h2>
            <div className="grid grid-cols-6 gap-2 text-xs font-medium text-foreground/70">
              {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                <div key={day} className="rounded-xl bg-primary/10 px-3 py-2 text-center uppercase tracking-wide">
                  {day}
                </div>
              ))}
            </div>
              ...
              {["08:00", "09:45", "11:30", "13:15"].map((time, index) => (
                <div key={time} className="grid grid-cols-6 gap-2">
                  {new Array(6).fill(null).map((_, dayIndex) => (
                    ...
                    >
                      <p className="text-xs font-semibold">Section {String.fromCharCode(65 + ((index + dayIndex) % 3))}</p>
      ...
      <section className="rounded-[2.5rem] border border-white/40 bg-white/80 p-10 shadow-card">
        <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-lg text-sm text-foreground/70">
            Capture every section, distribute contact hours evenly, and adapt instantly when calendars shift. Local
            storage keeps your draft aligned across sessions until you are ready to export.
          </p>
        </div>
